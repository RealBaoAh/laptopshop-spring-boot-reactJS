package vn.baoanh.laptopshop.service;
...
import vn.baoanh.laptopshop.repository.CartDetailRepository;
import vn.baoanh.laptopshop.repository.CartRepository;
import vn.baoanh.laptopshop.repository.OrderDetailRepository;
import vn.baoanh.laptopshop.repository.OrderRepository;
import vn.baoanh.laptopshop.repository.ProductRepository;
import vn.baoanh.laptopshop.service.specification.ProductSpecs;
...
public class ProductService {
    ...
    public synchronized Map<String, Long> getFactoryCounts() {
    
    List<Object[]> results = this.productRepository.countProductsByFactory();

    Map<String, Long> factoryCounts = new HashMap<>();
    List<String> defaultFactories = Arrays.asList("APPLE", "ASUS", "DELL", "HP", "ACER", "LENOVO");
    for (String factory : defaultFactories) {
        factoryCounts.put(factory, 0L);
    }
    
    if (results != null) {
        for (Object[] result : results) {
            
            String factory = (String) result[0];
            Long count = (Long) result[1];
            if (factory != null) {
                factoryCounts.put(factory.trim().toUpperCase(), count);
            }
        }
    }
